...
```
npm run build -- --name=cuelume --type=wwobject
```

Both `--name` and `--type` are required, and so is the bare `--` — without it npm swallows `--name` as its own config and never forwards it. The build exits 0 either way, so verify it actually ran by checking that `dist/manager.js` has this run's timestamp; a stale bundle next to a green exit is what missing args look like. (The leading dashes on the args need `@weweb/cli` 3.8.14 or newer; older versions want `name=cuelume type=wwobject`.)

This produces `dist/manager.js`, the editor bundle — `wwEditor` blocks are intentionally retained there. The production bundle that strips them is generated by WeWeb's pipeline on publish, so this build doesn't verify the editor-state-free path.
